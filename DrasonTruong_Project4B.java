...
public class DrasonTruong_Project4B extends Application {
	// Project 4B
	// ITSS 3312.001
	// Create GUI for 4 Dice Poker, include event handling
	// UNRESOLVED BUG: game must use reroll at least once in order to correctly display results
	// can click reroll with no radio buttons selected to keep intial roll
	// rerolling but clicking first roll button get new set of numbers for first roll, will save the new first roll numbers
	...
	public void start(Stage primaryStage) {
		...
		tfName.setOnKeyPressed(e-> { // save name from text field
			String name;
			name = tfName.getText();
			session.setName(name);
		});
		
		firstRollButton.setOnAction(e-> { // first roll button
			hands.setDice(); // get dice values, set labels to values
...
class Hands { // Class Hands - contains methods to evaluate hands 
	...
	// methods to return specific index values
	...
	public void rerollArray (int[] array) {
		Random random = new Random();
		newArray = array;
		if(rb1_on) { // if button has been selected will reroll value
	...
	//returns new array with new rerolled values
	...
	// method checks if button has been pressed
...
class Session { // Class Session - contains user specific items (name, number of games, results of games) 
